...
import { saveWeeklyExamResult } from '../../lib/firestore';
import { getWeeklyExamQuestions, calculateScore, getCategoryLabel, getCategoryColor, getWeekKey } from '../../lib/quiz';
...
export default function WeeklyExam() {
  ...
  const questions = useMemo(() => getWeeklyExamQuestions(), []);
  ...
  async function nextQuestion(lastPts: number, corrects: number) {
    ...
    if (next >= questions.length) {
      ...
      if (user) {
        saveWeeklyExamResult(user, finalScore, corrects, questions.length)
          .then((res) => setPercentile(res.percentile))
          .catch(() => {});
      }
    } else {
      setSelected(null);
  ...
  async function handleShare() {
    ...
    const pctText = percentile !== undefined ? ` — Türkiye genelinde ilk %${100 - percentile}'desin!` : '';
    await captureAndShare(shareCardRef, `Haftalık Deneme'de ${totalScore} puan yaptım${pctText} (${week})`);
  }
  ...
  if (finished) {
    ...
    return (
      ...
      >
        ...
        <View ref={shareCardRef} collapsable={false} style={[styles.resultCard, { backgroundColor: '#4338CA' }]}>
          <Text style={styles.resultEmoji}>{emoji}</Text>
          <Text style={styles.resultTitle}>Haftalık Deneme Bitti!</Text>
          ...
          <Text style={styles.shareWatermark}>Haftalık Deneme · {getWeekKey()}</Text>
        </View>
        ...
        {percentile !== undefined && (
          <View style={[styles.percentileChip, { backgroundColor: Colors.success + '22' }]}>
            ...
            <Text style={[styles.percentileText, { color: Colors.success }]}>
              Türkiye genelinde ilk %{100 - percentile}'desin 🎯
            </Text>
          </View>
        )}
        ...
        >
          ...
          <Text style={[styles.shareBtnText, { color: Colors.primary }]}>Sonucu Paylaş</Text>
        </TouchableOpacity>

        <Text style={[styles.resultMsg, { color: c.textSecondary }]}>
          {percentage >= 80
            ? 'Mükemmel! Gerçek sınav formatında çok iyiydin.'
            : percentage >= 60
            ? 'İyi iş! Gelecek Pazar daha iyisini yapabilirsin.'
            : 'Her Pazar tekrar dene, gelişimini takip et!'}
        </Text>
...
const styles = StyleSheet.create({
  ...
  resultScrollContent: { paddingTop: 12, paddingBottom: 40 },
  resultCard: {
  ...
  resultMsg: { textAlign: 'center', fontSize: 15, paddingHorizontal: 32, lineHeight: 22, marginTop: 12 },
